...
def _pg():
    """Open a Postgres connection to THIS project's database (cascade profile).
    Hard-coded on purpose: no PG* env vars (a stray PGUSER in the environment would
    otherwise hijack `run.sh setup`'s own psql before that role exists)."""
